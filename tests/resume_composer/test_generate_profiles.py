...
"""
 ...
 researcher , academic """
        ...
        assert countries == ["USA", "UK", "Germany"]
        assert profiles == [["data_scientist", "finance"], ["researcher", "academic"]]
    ...
    def test_read_profiles_file_no_profiles(
...
class TestGenerateResume:
    ...
    def test_generate_resume_success(
        self, mock_process: Mock, caplog: pytest.LogCaptureFixture
    ) -> None:
    ...
    @patch("resume_composer.generate_profiles.process_resume_substitution")
    def test_generate_resume_failure(
        self, mock_process: Mock, caplog: pytest.LogCaptureFixture
    ) -> None:

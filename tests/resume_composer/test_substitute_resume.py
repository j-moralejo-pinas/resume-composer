...
class TestResumeSubstituter:
    ...
    def test_get_value_for_placeholder_with_matching_tag(self, tmp_path: Path) -> None:
        ...
        assert substituter._get_value_for_placeholder("1", ["nonexistent", "academic"]) == "Dr. John Doe"
    ...
    def test_list_available_tags(self, tmp_path: Path, capsys: pytest.CaptureFixture[str]) -> None:
        ...
        assert "data_scientist" in captured.out

    def test_list_available_tags_empty(self, tmp_path: Path, capsys: pytest.CaptureFixture[str]) -> None:
        ...
        captured = capsys.readouterr()
        assert "No tags found in configuration." in captured.out


class TestProcessResumeSubstitution:
    """Test cases for the process_resume_substitution function."""

    def test_list_tags_mode(self, tmp_path: Path, capsys: pytest.CaptureFixture[str]) -> None:
    ...
    def test_substitution_with_country(self, tmp_path: Path) -> None:
        ...
        config_data = {
            "1": {"default": "John", "uk": "John Smith UK"}
        }
        config_file = tmp_path / "config.json"
        config_file.write_text(json.dumps(config_data))
...
class TestCompileLatexToPdf:
    ...
    def test_compile_nonexistent_file(self, capsys: pytest.CaptureFixture[str]) -> None:
        """Test compilation of nonexistent file."""
        compile_latex_to_pdf("nonexistent.tex")
        
        captured = capsys.readouterr()
        assert "Error: LaTeX file 'nonexistent.tex' not found." in captured.out

    @patch('resume_composer.substitute_resume.subprocess.run')
    def test_compile_successful(self, mock_run: Mock, tmp_path: Path, capsys: pytest.CaptureFixture[str]) -> None:
        ...
        captured = capsys.readouterr()
        assert f"Compiling {tex_file} to PDF..." in captured.out
        assert f"Successfully compiled to '{pdf_file}'" in captured.out

    @patch('resume_composer.substitute_resume.subprocess.run')
    def test_compile_failure(self, mock_run: Mock, tmp_path: Path, capsys: pytest.CaptureFixture[str]) -> None:
        ...
        captured = capsys.readouterr()
        assert "Compilation failed with return code 1" in captured.out
        assert "Error output:" in captured.out

    @patch('resume_composer.substitute_resume.subprocess.run')
    def test_compile_timeout(self, mock_run: Mock, tmp_path: Path, capsys: pytest.CaptureFixture[str]) -> None:
        ...
        captured = capsys.readouterr()
        assert "Compilation timed out after 60 seconds." in captured.out

    @patch('resume_composer.substitute_resume.subprocess.run')
    def test_compile_pdflatex_not_found(self, mock_run: Mock, tmp_path: Path, capsys: pytest.CaptureFixture[str]) -> None:
        ...
        captured = capsys.readouterr()
        assert "Error: pdflatex not found. Please install a LaTeX distribution." in captured.out
